 ...
 */
function RespondEmail_SendAgain(e) {
    ...
    if (emailQuotaRemaining > 0) {
        ...
        var threads = GmailApp.search("label:read subject:'Inquiry from'", 0, 100);
        ...
        for (var i = 0; i < threads.length; i++) {
            ...
            client_mail_array = ['cphgsales@gmail.com', 'cphgsales@hotmail.com', 'lvnvac@gmail.com', 'lvnvac@outlook.com', 'lvnvacation@gmail.com', 'pete@cpi-homes.com'];
                        ...
                        if (property_id > 0) {
                            ...
                            if (data.status == "success") {
                               ...
                               // responsebody += "http://www.lvnvacationhomerentals.com/?chki=" + data.start_date + "&chko=" + data.stop_date;
                                ...
                                var CurrentDateString1 = Utilities.formatDate(CurrentDate,  Session.getScriptTimeZone(), "MM/dd/yyyy"); 
                                var last_msg_Date = Utilities.formatDate(threads[i].getLastMessageDate(),Session.getScriptTimeZone(), "MM/dd/yyyy"); 
                                
                                var date1 = new Date(CurrentDateString1);
                                var date2 = new Date(last_msg_Date);
                                var timeDiff = Math.abs(date2.getTime() - date1.getTime());
                                var diffDays = Math.ceil(timeDiff / (1000 * 3600 * 24)); 
                                Logger.log(diffDays + 'Day differnc hey');
                                 var diffDays_check = diffDays
                              
                                Logger.log(CurrentDateString1 + 'Current date');
                                Logger.log(last_msg_Date + 'Last message date');
                                odayDate = new Date();

                                if (threads[i].getMessageCount() == 2 && diffDays_check >=3 ) {
                                    ...
                                    if ((reg.test(client_email) == false)) {
                                        ...
                                        //threads[i].reply("", {htmlBody: responsebody,from:client_email});
                                        //threads[i].markRead();
