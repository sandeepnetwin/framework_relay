...
class Analog extends CI_Controller 
{
	...
	public function removeLight()
	{
		...
		$aLightDetails  =   $this->home_model->getLightDeviceDetails($lightNumber);
		if(!empty($aLightDetails))
		{
			...
			if($sRelayType == '24')
			{
				...
				if($sLightStatus)
				{
					$sNewResp = replace_return($sRelays, "0", $sRelayNumber );
					onoff_rlb_relay($sNewResp);
					$this->home_model->updateDeviceRunTime($sRelayNumber,'R',"0");
				}
		...
		$this->load->model('analog_model');
		$this->analog_model->removeDeviceDetails($lightNumber,'L');
		exit;
	}
	...
	public function removeBlower()
	{
		...
		$aBlowerDetails  =   $this->home_model->getBlowerDeviceDetails($blowerNumber);
		...
		//Remove the light from database.
		$this->load->model('analog_model');
		$this->analog_model->removeDeviceDetails($blowerNumber,'B');
		exit;
	}
}
